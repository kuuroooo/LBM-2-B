document.addEventListener('DOMContentLoaded', function() {
    ...
    // Function to update data for a specific polygon
    ...
    // var southWest = L.latLng(52.248553, 4.839145);
    // var northEast = L.latLng(52.337397, 4.857643);
    // var bounds = L.latLngBounds(southWest, northEast);

    // // Set maximum bounds
    // map.setMaxBounds(bounds);

    // L.tileLayer('https://api.maptiler.com/maps/nl-cartiqo-light/{z}/{x}/{y}.png?key=0MDV6Trq9vv0OTbxV1dZ', {
    ...
    // Define a function to find the polygon layer based on the area name
    function findPolygonLayer(areaName) {
        // Use a mapping to find the corresponding polygon layer
        switch (areaName) {
            case "Stadshart":
                return stadshart;
            case "Randwijck":
            ...
            case "Elsrijk":
                return elsrijk;
            case "Uilenstede/Kronenburg":
                return uilenstede_kronenburg;
            case "Bankras/Kostverloren":
                return bankras_kostverloren;
            case "Keizer Karelpark":
                return keizer_karelpark;
            case "Groenelaan":
                return groenelaan;
            case "Waardhuizen/Middenhoven":
                return waardhuizen_middenhoven;
            case "Bovenkerk/Westwijk Noord":
                return bovenkerk_westwijknoord;
            case "Westwijk South":
                return westwijksouth;
            case "Amsterdamse Bos":
                return amsterdamsebos;
            case "Buitengebied Nord":
                return buitengebiednoord;
            case "Buitengebied Zuid":
                return buitengebiedzuid;
            default:
                return null; 
        }
    }
    ...
    async function fetchData(wk_naam_input, jaar_input) {
        const response = await fetch('AmstelveenData.csv');
        ...
        // Loop through each row and find the matching row
        for (let i = 1; i < rows.length; i++) { // Start from index 1 to skip header row
            const columns = rows[i].split(';');

            // Extracting columns
            const wk_naam = columns[1];
            const jaar = parseInt(columns[2]);
    ...
    function getDynamicCategories(min, max) {
        const range = max - min;
        const step = range / 9;
        const categories = [];
        for (let i = 0; i <= 9; i++) {
            categories.push(min + i * step);
        }
        return categories;
    }
    ...
    function assignColor(score) {
        ...
        const categoriesLBM = [3.5, 3.625, 3.75, 3.875, 4, 4.125, 4.25, 4.375, 4.5];
        ...
        // const colors = ['#f44e25', '#f36400', '#f07800', '#e88c00', '#dca100', '#cbb406', '#b7c629', '#9cd649', '#79e66b'];

        // Find the category index for the given score
        let categoryIndex = 0;
        while (categoryIndex < categories.length && score > categories[categoryIndex]) {
            categoryIndex++;
        }

        // Return the color corresponding to the category
    ...
    function getUserWeights() {
        return {
            'fys': parseInt(document.getElementById('p').value),
            'onv': parseInt(document.getElementById('s&n').value),
            'soc': parseInt(document.getElementById('sc').value),
            'vrz': parseInt(document.getElementById('s&n').value),
            'won': parseInt(document.getElementById('hs').value)
        };
    };
...
document.getElementById('reset-button').addEventListener('click', function() {
    // Reset all sliders to the default value of 3
    ...
    // Trigger the slider change handler to update the map
    handleSliderChange();
});